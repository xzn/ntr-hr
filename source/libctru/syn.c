...
{
	return svc_createAddressArbiter(&arbiter);
}

void __sync_fini(void)
{
	if (arbiter)
		svc_closeHandle(arbiter);
}

Result syncArbitrateAddress(s32* addr, ArbitrationType type, s32 value)
{
	return svc_arbitrateAddressNoTimeout(arbiter, (u32)addr, type, value);
}

Result syncArbitrateAddressWithTimeout(s32* addr, ArbitrationType type, s32 value, s64 timeout_ns)
{
	return svc_arbitrateAddress(arbiter, (u32)addr, type, value, timeout_ns);
}
