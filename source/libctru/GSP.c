...
static void gspEventThreadMain(u32);
...
Result gspInit(u32 *threadStackBase, u32 acquireRight)
{
	...
	if (R_FAILED(ret)) {
		nsDbgPrint("gsp get srv failed: %08x\n", ret);
		goto _fail0;
	}
	...
	if (R_FAILED(ret)) {
		nsDbgPrint("gsp register interrupt failed: %08x\n", ret);
		goto _fail2;
	}

	// Initialize the hardware if we are the first process to register
	if (ret == 0x2A07) {
		nsDbgPrint("gsp Hardware Init\n");
		gspHardwareInit();
	}

	// Map GSP shared memory
	gspSharedMem = mappableAlloc(0x1000);
	ret = svcMapMemoryBlock(gspSharedMemHandle, (u32)gspSharedMem, MEMPERM_READWRITE, MEMPERM_DONTCARE);
	if (ret != 0) {
		nsDbgPrint("map memory block failed: %08x\n", ret);
	}
...
void gspExit(void)
{
	...
	svc_waitSynchronization1(gspEventThread, U64_MAX);
